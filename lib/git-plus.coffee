...
GitDiffBrancheFiles    = require './models/git-diff-branch-files'
  ...
  activate: (state) ->
    ...
    if repos.length > 0
      ...
      if atom.config.get('git-plus.diffs.splitDiff')
        @subscriptions.add atom.commands.add 'atom-workspace', 'git-plus:diff-branch-files', -> git.getRepo().then((repo) -> GitDiffBrancheFiles(repo))
